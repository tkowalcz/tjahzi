...
The `log4j2-appender`, `logback-appender` and `reload4j-appender` artifacts
ship GraalVM reachability metadata and work in native images out of the box:

- Reflection configuration for the appender and its nested configuration
  elements is embedded in each jar (for log4j2 it is generated by the
  log4j plugin annotation processor).
- All Tjahzi classes are marked for run time initialization. The off heap
  log buffer and the log shipper thread are created when the logging
  system starts, so the logging framework must not be initialized at image
  build time.
- Remember to include your logging configuration file as a resource in the
  image (e.g. `logback.xml` via a `resource-config.json`).

The shaded `*-nodep` artifacts are not supported in native images - class
relocation invalidates the bundled Netty reachability metadata. Use the
regular artifacts instead.

Native image support is exercised in CI: a smoke test application is
compiled with `native-image` and verified end to end against a real Loki
instance (see the `native-image-test` module, built with `-Pnative`).
