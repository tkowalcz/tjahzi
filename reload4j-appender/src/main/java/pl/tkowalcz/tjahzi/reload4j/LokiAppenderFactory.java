...
public class LokiAppenderFactory {
    ...
    public LoggingSystem createAppender() {
        ...
        if (configurator.isVerbose()) {
            monitoringModuleWrapper.setMonitoringModule(new LoggingMonitoringModule(LogLog::error));
        } else {
        ...
        int bufferSizeBytes = configurator.getBufferSizeMegabytes() * BYTES_IN_MEGABYTE;
